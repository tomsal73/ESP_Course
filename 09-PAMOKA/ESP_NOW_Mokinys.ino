...
#define LEDPIN 25     // Lemputės jungtis, pakeiskite skaičių, jei naudojate kitą jungtį
...
// Pakeiskite mokytojo MAC Adresą
uint8_t addrMokytojas[] = {0x78, 0x21, 0x84, 0x80, 0x20, 0x18};
...
void setup() {
  ...
  if (esp_now_init() == ESP_OK) {
     ...
     memcpy(peerInfo.peer_addr, addrMokytojas, 6);
     peerInfo.channel = 0;  
  ...
  // Siunčiam pranešimą mokytojui, kad mane užregistruotų
  // Nustatom siunčiamų duomenų reikšmes
  // Vardą pakeiskite į savo vardą
  strcpy(myData.vardas, "Tomas");
  strcpy(myData.txt, "Tuk tuk") ;

  // Siunčiam pranešimą per ESP-NOW
  esp_err_t result = esp_now_send(addrMokytojas, (uint8_t *) &myData, sizeof(myData));
...
void loop() {
  ...
  if (dataRecived) {
     // Uždegam arba užgesinam lempute
     if (strcmp(receivedData.txt,"Sensorius")==0) {
        digitalWrite (LEDPIN, receivedData.skaicius) ;
     }   
     sprintf (statusTxt, "Gauta nuo '%s': %s", receivedData.vardas, receivedData.txt) ; 
     Print3(statusTxt);
     dataRecived = false ;
  }
