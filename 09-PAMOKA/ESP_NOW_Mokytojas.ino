...
// Siuntejo MAC adresas
uint8_t addrSiuntejas[] = {0x00, 0x00, 0x0, 0x00, 0x00, 0x00};
...
void loop() {
  ...
  if (dataRecived) {
    ...
    // Registruojam siuntėjo adresą
     memcpy(peerInfo.peer_addr, addrSiuntejas, 6);
     peerInfo.channel = 0;  
    ...
    strcpy(myData.vardas, "Mokytojas");
    strcpy(myData.txt, "Jus uzregistruotas") ;
    esp_err_t result = esp_now_send(addrSiuntejas, (uint8_t *)&myData, sizeof(myData));
  ...
  // Siunčiam sensoriaus duomenis visiems dalyviams
  strcpy(myData.vardas, "Mokytojas");
  strcpy(myData.txt, "Sensorius") ;
  myData.skaicius = digitalRead (SENSORPIN) ;
  // pirmas parametras 0 nurodo, kad bus siunčiama visiems
